...
open class Button: UIButton {
    ...
    */
    ...
    /**
     Observer for observing state change of activityIndicator, in case
     it is handled from outside using Observable pattern.
     */
    private lazy var activityIndicatorObserver
        = Observer(onValueChanged: activityIndicatorValueChanged)
    ...
    public convenience init(
        ...
        activityIndicatingObservable: Observable<Bool>? = nil
    ) {
        ...
        activityIndicatingObservable?.addObserver(activityIndicatorObserver)
    }
    ...
    private func activityIndicatorValueChanged(isAnimating: Bool) {
        if isAnimating {
            showLoader(userInteraction: false)
        } else {
            hideLoader()
        }
    }
